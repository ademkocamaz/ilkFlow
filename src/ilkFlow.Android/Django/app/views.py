...
def index(request):
    ...
    user_activities=Log.objects.filter(user=request.user)[:10]
...
def settings(request):
    ...
    states=State.objects.filter(user=request.user).order_by('created_date')
    context={
...
def category(request):
    ...
    categories = Category.objects.filter(user=request.user).order_by('-created_date')
...
def category_detail(request,category_id):
    category=get_object_or_404(Category,pk=category_id)
    processes=Process.objects.filter(user=request.user).filter(category=category)
...
def process(request):
    ...
    processes = Process.objects.filter(user=request.user).order_by('-created_date')
    context = {
        'process_form':process_form,
        'processes': processes
    }
...
def activity(request):
    ...
    activities = Activity.objects.filter(user=request.user).order_by('-created_date')
    context = {
...
def my_tasks(request):
    ...
    tasks = Task.objects.filter(user=request.user).filter(assigned_user=request.user).order_by('-created_date')
    context = {
        'task_form':task_form,
        'tasks': tasks
    }
